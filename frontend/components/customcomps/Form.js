...
import { Alert, DeviceEventEmitter, Keyboard, ScrollView, StyleSheet, Text, View } from "react-native"
...
const fetchCity = () =>{
  ...
  if(cityData.features[0] !== undefined && cityData.features[0].properties.name === undefined){
    insert(new Blog(undefined, inputs.title, inputs.city , imageUri ,inputs.body))
...
const submitFormHandler = async() =>{
  Keyboard.dismiss()
const res = await  fetch(`${URL}/${inputs.city}`)
 ...
 if(result){
  ...
  return (
    ...
    <CustomInput style={!cityError? null: styles.inputError  } inputConfig={{
      value:inputs.city,
